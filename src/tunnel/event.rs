...
impl Event {
    pub fn new() -> Self {
        Event {
            waker_id: EVENT_ID.fetch_add(1, std::sync::atomic::Ordering::Relaxed),
    ...
    pub fn set(&self) -> Result<(), &'static str> {
        ...
        // To avoid deadlocks, we need to release the lock before waking up the wakers
        {
            ...
            if state_guard.value {
                return Ok(());
            }
            state_guard.value = true;
            ...
            awakers = state_guard.wakers.drain().map(|(_, waker)| waker).collect();
...
impl Drop for Event {
    fn drop(&mut self) {
        ...
        state_guard.wakers.remove(&self.waker_id);
    }
}
...
impl Future for Event {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if state_guard.value {
            ...
            state_guard.wakers.insert(self.waker_id, cx.waker().clone());
            Poll::Pending
        }
    }
}
